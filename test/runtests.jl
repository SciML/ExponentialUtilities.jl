...
@testset "Arnoldi & Krylov" begin
    ...
    @test W ≈ W_approx
    W_approx_kiops3 = kiops(t, A, hcat([b * inv(t)^i for i = 0:K-1]...))
    @test sum(W[:, 1:K], dims = 2) ≈ W_approx_kiops3[1]
    @test_skip begin
        W_approx_kiops4 = kiops(t, A, hcat([b * inv(t)^i for i = 0:K]...))
...
LinearAlgebra.mul!(y::AbstractVector, A::MatrixFreeOperator, x::AbstractVector) =
    mul!(y, A.A, x)
...
@testset "Matrix-free Operator" begin
    ...
    for ishermitian in (false, true)
        ...
        b = rand(ComplexF64, n)
        Ks =
            arnoldi(Op, b; ishermitian = ishermitian, opnorm = OpnormFunctor(), tol = 1e-12)
        pv = phiv(0.01, Ks, 2)
