...
"""
    kiops(tau_out, A, u; kwargs...) -> (w, stats)

Evaluate a linear combination of phi functions at the requested output times
using the adaptive KIOPS Krylov algorithm:
...
# Arguments
  ...
  - `tau_out`: scalar or ordered collection of output times. All entries must
    have the same sign.
  - `A`: square matrix or linear operator used as the phi-function argument.
  - `u`: vector `u_0`, or matrix whose columns are `u_0, u_1, ...` in the
    linear combination above.

# Keywords

  - `tol::Real = 1e-7`: requested local convergence tolerance.
  - `mmin::Int = 10`: minimum adaptive Krylov dimension.
  - `mmax::Int = 128`: maximum adaptive Krylov dimension.
  - `m::Int = min(mmin, mmax)`: initial Krylov dimension estimate.
  - `iop::Int = 2`: incomplete-orthogonalization length.
  - `ishermitian::Bool = LinearAlgebra.ishermitian(A)`: whether `A` is
    Hermitian. Only set this to `true` when the operator obeys that contract.
  - `opnorm = LinearAlgebra.opnorm(A, Inf)`: operator-norm scale used by the
    adaptive controller.
  - `task1::Bool = false`: apply the KIOPS task-one final scaling when `true`.

# Returns

Returns `(w, stats)`, where `w` contains the requested linear combinations and
`stats` is the tuple `(substeps, rejected_steps, krylov_steps,
matrix_exponentials, final_krylov_dimension)`.
