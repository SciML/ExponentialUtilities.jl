...
@inline function exp_14(A)
    T=promote_type(eltype(A),Float64)
    A_copy=similar(A,T); A_copy .= A;
    return exp_14!(cache,A_copy)
end

@inline function exp_14!(cache,A)
    ...
    # Computation order: C A2 A4 A6 Ub3 Ub Uc U Vb3 Vb V Z X P S1 S2 S3 S4 S5 S6 S7 S8 S9
    # Computing C = x*A+x*I
    coeff1=0.001953125
    coeff2=0.0
    ...
    # Computing S8 with operation: mult
    mul!(memslots2,memslots1,memslots1)
    # Deallocating S7 in slot 1
    # Computing S9 with operation: mult
    mul!(memslots1,memslots2,memslots2)
    # Deallocating S8 in slot 2
    return memslots1 # Returning S9
end
