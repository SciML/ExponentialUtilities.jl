...
@generated function naivemul!(
    ...
    nrem_body = quote
        ...
        while m < M - $(MU - 1)
            ...
            for k ∈ Kaxis
                Base.Cartesian.@nexprs $MU i ->
                    Cmn_i = muladd(_const(A)[m+i, k], _const(B)[k, nn], Cmn_i)
            end
        ...
        Base.Experimental.@aliasscope begin
            ...
            @inbounds begin
                while n < N - $(NU - 1)
                    m = first(Maxis) - 1
                    while m < M - $(MU - 1)
                        Base.Cartesian.@nexprs $NU j ->
                            Base.Cartesian.@nexprs $MU i -> Cmn_i_j = zero(T)
                        for k ∈ Kaxis
                            Base.Cartesian.@nexprs $MU i -> Ak_i = _const(A)[m+i, k]
                            Base.Cartesian.@nexprs $NU j -> begin
                                Bk_j = _const(B)[k, n+j]
                                Base.Cartesian.@nexprs $MU i ->
                                    Cmn_i_j = muladd(Ak_i, Bk_j, Cmn_i_j)
                            end
                        end
                        Base.Cartesian.@nexprs $NU j ->
                            Base.Cartesian.@nexprs $MU i -> C[m+i, n+j] = Cmn_i_j
                        m += $MU
                    end
                    for mm ∈ 1+m:M
                        ...
                        for k ∈ Kaxis
                            Base.Cartesian.@nexprs $NU j ->
                                Cmn_j = muladd(_const(A)[mm, k], _const(B)[k, n+j], Cmn_j)
                        end
...
function exp_pade_p(x, ::Val{13}, ::Val{13})
    @evalpoly(
        x,
        LinearAlgebra.UniformScaling{Float64}(1.0),
...
function exp_pade_p!(
    ...
    for c ∈ (
        2.8101705462199623e-15,
        2.529153491597966e-13,
        1.48377004840414e-11,
        6.306022705717595e-10,
        2.0431513566525008e-8,
        5.175983436853002e-7,
        1.0351966873706003e-5,
